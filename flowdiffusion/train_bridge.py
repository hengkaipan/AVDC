...
from datasets_avdc_avdc import SequentialDatasetNp, SequentialDatasetVal
import argparse
...
def main(args):
    ...
    if args.mode == "train":
        ...
        output = [
            np.array(Image.fromarray(frame).resize((320, 240))) for frame in output
        ]
...
if __name__ == "__main__":
    ...
    )  # 'train for training, 'inference' for generating samples
    parser.add_argument(
        "-c", "--checkpoint_num", type=int, default=None
    )  # checkpoint number to resume training or generate samples
    parser.add_argument(
